...
function publicUser(user) {
  return {
    ...
    totalSitesGenerated: user.totalSitesGenerated,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // password is select:false, so opt in explicitly for the comparison.
...
// ===== PROFILE =====
// These read req.user, so they MUST sit behind authMiddleware. Mounting the
// whole router without it was crashing both endpoints with a 500.
router.get(
  '/profile',
  authMiddleware,
  asyncHandler(async (req, res) => {
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    const updates = {};
    for (const field of ['name', 'phone', 'businessName']) {
